...
{
    public partial class ProjectSoldierSuperAdmin : DevExpress.XtraBars.FluentDesignSystem.FluentDesignForm
    {

        SqlConnection conn = Config.connString.conn;
        public ProjectSoldierSuperAdmin()
        {
            InitializeComponent();                     
        }
        ...
        {
            Connection();
            SqlCommand com = new SqlCommand("SELECT ListOfTrainings.Title, UserTrainings.RequestStat, UserTrainings.UserID, ListOfTrainings.TrainingID FROM UserTrainings inner join ListOfTrainings on UserTrainings.TrainingID = ListOfTrainings.TrainingID where UserTrainings.RequestStat = 'Pending' or  UserTrainings.RequestStat = 'Accepted'", conn);
            SqlDataAdapter adapter = new SqlDataAdapter(com);
            DataTable table = new DataTable();
            adapter.Fill(table);
        ...
        {
            Connection();
            SqlCommand com = new SqlCommand("SELECT ListOfTrainings.Title as Training_Title, concat (AntiqueUsers.FName,' ',AntiqueUsers.MName,' ',AntiqueUsers.LName,' ',AntiqueUsers.NameExt) as Trainees, UserTrainings.UserID, ListOfTrainings.TrainingID, UserTrainings.TraineeApperance as Attendance FROM ((AntiqueUsers inner join UserTrainings on AntiqueUsers.UserID = UserTrainings.UserID) inner join ListOfTrainings on UserTrainings.TrainingID = ListOfTrainings.TrainingID) Where UserTrainings.RequestStat = 'Accepted'", conn);
            SqlDataAdapter adapter = new SqlDataAdapter(com);
            DataTable table = new DataTable();
            adapter.Fill(table);
            gcTraineesAttendance.DataSource = table;
            dgvTraineesAttendance.Columns[2].Visible = false;
            dgvTraineesAttendance.Columns[3].Visible = false;
        ...
        {
            // TODO: This line of code loads data into the 'dataSet_ListOfUsers.AntiqueUsers' table. You can move, or remove it, as needed.
           // this.antiqueUsersTableAdapter.Fill(this.dataSet_ListOfUsers.AntiqueUsers);
            //try
            //{
        ...
        {
            Connection();
            SqlCommand com = new SqlCommand("Select * from AntiqueUsers",conn);
            SqlDataAdapter adapter = new SqlDataAdapter(com);
            DataTable table = new DataTable();
            adapter.Fill(table);
            gcListOfRegisteredUsers.DataSource = table;
        }
        ...
        private void certAttendancePrintBtn_Click(object sender, EventArgs e)
        {
            ...
            Qfrm.tbUserType.Text = "Super Admin";
            Qfrm.ShowDialog();
            //addQRImageAttendance();
